...
public class DbLogHandler {
	...
	public void LogErrorTypeAndError(String errorid, String conversationid, String error){
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = "insert into sdplog.log (messageid, conversationid, error) values(?,?,?)";
		try {
			con = dbConnect();
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, new Integer(errorid).intValue());
